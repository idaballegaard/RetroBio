...
// Hent visninger (server-side injiceret i window)
const showings = Array.isArray(window.showings) ? window.showings : [];
...
bookBtn.className = "mt-4 ml-auto bg-amber-400 text-black font-bold px-8 py-3 rounded-lg text-lg shadow-md transition cursor-not-allowed opacity-50";
bookBtn.addEventListener('click', () => {
  if (!selectedShowtime) return;
  window.location.href = `book-tickets.php?movie=${encodeURIComponent(movieInput.value)}&time=${selectedShowtime.startTime}`;
});
...
document.addEventListener('click', (e) => { 
  if (!input.contains(e.target) && !calendar.contains(e.target)) calendar.classList.add('hidden'); 
});
...
function updateShowtimes() {
  ...
  // Ingen film valgt
  if (!movieName) {
    showtimesDiv.innerHTML = '<p class="text-gray-400 text-sm italic">Select a movie first.</p>';
    showtimesDiv.appendChild(bookBtn);
    return;
  }
...
function updateShowtimes() {
  ...
  // Sørg for at Book Tickets-knappen altid eksisterer
  bookBtn.disabled = true;
  bookBtn.classList.remove('glow');
  bookBtn.classList.add('opacity-50', 'cursor-not-allowed');

  // Hvis der ikke er valgt film eller dato
  if (!movieName && !dateVal) {
    showtimesDiv.innerHTML = '<p class="text-gray-400 text-sm italic">Select a movie and date to view showtimes.</p>';
    showtimesDiv.appendChild(bookBtn);
    return;
  }

  // Hvis der er valgt film, men ikke dato
  if (movieName && (!dateVal || weekday === null)) {
    showtimesDiv.innerHTML = '<p class="text-gray-400 text-sm italic">Choose a date to see available showtimes.</p>';
    showtimesDiv.appendChild(bookBtn);
    return;
  }

  // Filtrér showings for valgt film og dato
  const timesForDay = showings.filter(s => {
    const d = new Date(s.date);
    const chosen = new Date(dateVal);
    return (
      s.movieTitle === movieName &&
      d.getFullYear() === chosen.getFullYear() &&
      d.getMonth() === chosen.getMonth() &&
      d.getDate() === chosen.getDate()
    );
  });
  ...
  timesForDay.forEach(t => {
    ...
    showtimesDiv.appendChild(card);
  });

  // Tilføj knappen til sidst uanset hvad
  showtimesDiv.appendChild(bookBtn);
}