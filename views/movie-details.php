...
?>
    ...
    <style>
        @keyframes pulseGlow {
            0%, 100% {
                box-shadow: 0 0 10px #fbbf24, 0 0 20px #fbbf24, 0 0 40px #fbbf24;
            }
            50% {
    ...
    <div class="max-w-6xl mx-auto px-6 pt-20 pb-20">
        ...
        <div class="flex flex-col md:flex-row md:items-center gap-6 mb-12">
            ...
            <div class="relative w-full md:w-1/2">
                <form method="get" action="<?= generateUrl('movie-details') ?>">
                    ...
                    <i data-feather="chevron-down"
                       class="absolute right-3 top-3 text-[#00e7ec] pointer-events-none"></i>
            ...
            <?php if($viewModel->getSelectedMovie()): ?>
                <!-- Date Dropdown -->
                <div class="relative w-full md:w-1/2">
                    <!--<form method="get" action="<?= generateUrl('movie-details') ?>">-->
            ...
            <section id="movieDetails" class="border border-[#00e7ec] rounded-2xl overflow-hidden shadow-lg">
                <div class="flex flex-col md:flex-row">
                    ...
                    <div class="md:w-2/3 p-8">
                        <h1 id="movieTitle"
                            class="text-4xl font-bold text-[#00e7ec] mb-3"><?= safeString($selectedMovie->getTitle()); ?></h1>
                        <div class="flex items-center mb-4">
                            <i data-feather="star" class="text-[#00e7ec] mr-2"></i>
                            <span id="movieRating"><?= safeString($selectedMovie->getRanking()); ?> / 5</span>
                        </div>
                        <p id="movieDesc"
                           class="text-gray-300 mb-6 leading-relaxed"><?= safeString($selectedMovie->getDescription()); ?></p>
                        <div class="grid grid-cols-2 md:grid-cols-3 gap-4 text-sm">
                            <div>
                                <span class="text-[#00e7ec]">Release Year:</span> <?= $selectedMovie->getReleaseYear(); ?>
                            </div>
                            <div>
                                <span class="text-[#00e7ec]">Genre:</span> <?= implode(', ', array_map(fn($g) => $g->getName(), $selectedMovie->getGenres())); ?>
                            </div>
                            <div><span class="text-[#00e7ec]">Length:</span> <?= $selectedMovie->getLength(); ?> min
                            </div>
                            <div>
                                <span class="text-[#00e7ec]">Language:</span> <?= safeString($selectedMovie->getLanguage()); ?>
                            </div>
